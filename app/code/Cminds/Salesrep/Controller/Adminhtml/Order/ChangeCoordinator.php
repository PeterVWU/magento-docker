 ...
 */
class ChangeCoordinator extends \Magento\Customer\Controller\Adminhtml\Index
{
    protected $salesrepRepositoryInterface;
    protected $adminUsers;

    /**
     * ChangeCoordinator constructor.
            ...
            if ($params['orderId']) {
                ...
                if ($params['coordinatorId']) {
                    $adminUser = $this->adminUsers
                        ->getItemById($params['coordinatorId']);
                } else {
                    $adminUser = false;
                }
                ...
                if ($adminUser) {
                    if ($adminUser->getSalesrepRepCommissionCalculationType() ==
                        ...
                        $coordinatorCommissionEarned = $this->salesrepRepositoryInterface
                            ->getManagementCommissionEarned(
                                $params['orderId'],
                                $adminUser->getSalesrepCoordinatorCommissionRate(),
                    ...
                    $salesrep
                        ->setCoordinatorId($adminUser->getId())
                        ->setCoordinatorName(
                            $adminUser->getData('firstname') . ' ' .
                            $adminUser->getData('lastname')
                        );
                    if ($coordinatorCommissionEarned >= 0) {
                        $salesrep->setCoordinatorCommissionEarned($coordinatorCommissionEarned);
                    }

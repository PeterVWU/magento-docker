    ...
    public function beforeLoad($printQuery = false, $logQuery = false)
    {
        ...
        if ($isModuleEnabled) {
            if ($printQuery instanceof \Magento\Customer\Model\ResourceModel\Grid\Collection) {
                ...
                if (!in_array('salesrep', $joined_tables) && !in_array('customer_entity_int', $joined_tables)) {
                    ...
                    if (!$isAdmin && !$view_rep_name_all) {
                        ...
                        if ($view_rep_name_sub) {
                            ...
                            if ($subordinateIds) {
                                $subordinateIdValues = $subordinateIds;
                            }
                        }

                        if ($view_own_customers) {
                            $subordinateIdValues[] = $this->authSession->getUser()->getId();
                        }

                        $collection->addFieldToFilter(
                            'customer_entity_int.value',
                            ['in' => $subordinateIdValues]
                        );
                    }
                }
            }

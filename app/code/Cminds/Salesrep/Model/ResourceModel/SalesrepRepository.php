...
class SalesrepRepository implements \Cminds\Salesrep\Api\SalesrepRepositoryInterface
{
    ...
    public function save(\Cminds\Salesrep\Api\Data\SalesrepInterface $salesrepInterface)
    {
        ...
        $data = $salesrepInterface->getData();

        /* array of fields that must be removed from $data if value is null to avoid Deprecated Functionality in
           vendor/magento/framework/Cache/Frontend/Adapter/Zend.php on line 123
        ...
        */
        ...
        foreach($data as $key => $value) {
            if (in_array($key, $fixStringArray) && is_null($value)) {
                unset($data[$key]);
            }
        }

        $salesrepData->setData($data);
        $salesrepData->save();
    ...
    public function getRepCommissionEarned($order_id, $rep_commission)
    {
        ...
        foreach ($order->getItems() as $orderItem) {
            ...
            if ($commissionType == CalculationType::BASE_PRICE_CALCULATION_TYPE) {
            ...
            if ($commission === null) {
                if ($groupId = (int)$product->getSalesrepRepCommissionGroup()) {
    ...
    protected function getGroupCommission(int $groupId)
    {
        ...
        return round((float)$commissionGroup->getCommissionRate(), 2);
    }
    ...
    public function getManagementCommissionEarned(
        ...
        foreach ($order->getItems() as $orderItem) {
            ...
            if ($commissionCalculationType == CalculationType::BASE_PRICE_CALCULATION_TYPE) {
                $price = $orderItem->getBasePrice();
            }

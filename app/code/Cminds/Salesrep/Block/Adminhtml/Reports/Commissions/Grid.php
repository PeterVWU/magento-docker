...
//            $orderStatuses = 'canceled,closed,complete,fraud,holded,payment_review,paypal_canceled_reversal,paypal_reversed,processing,pending';
        ...
        if (isset($orderAdmins) && is_array($orderAdmins)) {
            $cond = [];
            $cond[] = ['in' => explode(',', implode(',', $orderAdmins))];
    ...
    public function prepareReportData()
    {
        ...
        foreach ($collection as $row) {
            ...
            if (empty($filters)) {
                ...
                $orderAdmins = $filters['sales_rep'];
//                $orderAdminsArray = explode(',', $orderAdmins);
            }

            if ($row->getManagerId() && !empty($orderAdmins) && in_array($row->getManagerId(), $orderAdmins)) {
                $isManager = 1;    // manager
            ...
            // Coordinator
            if ($row->getCoordinatorId() && !empty($orderAdmins) && in_array($row->getCoordinatorId(), $orderAdmins)) {

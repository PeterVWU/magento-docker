...
namespace Cminds\Salesrep\Block\Adminhtml\Reports\Gross;
...
/**
 * Class Grid
 * @package Cminds\Salesrep\Block\Adminhtml\Reports\Gross
 */
class Grid extends \Magento\Reports\Block\Adminhtml\Grid\AbstractGrid
{
    ...
    {
        parent::_construct();
        $this->setTemplate('Cminds_Salesrep::report/gross/grid.phtml');
    ...
    public function getFilteredCollection()
    {
        ...
        if (empty($filters)) {
            ...
            if (isset($filters['order_statuses']) && is_array($filters['order_statuses'])) {
                $orderStatuses = implode(',', $filters['order_statuses']);
            }
            if (isset($filters['sales_rep'])) {
        ...
        foreach ($collection as $row) {
            ...
            if ($showName && ($rep_name = $row->getCoordinatorName()) != '') {
                ...
                if (strtolower($row->getCoordinatorCommissionStatus() ?? '') == "paid") {
                    ...
                    if (strtolower($row->getCoordinatorCommissionStatus() ?? '') == "unpaid") {
                        $data[$rep_name]['unpaid_total'] += round(
                            $row->getCoordinatorCommissionEarned(),
                            2
                        );

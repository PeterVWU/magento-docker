...
module "buckets" {
  ...
  project_id                               = var.project_id
  location                                 = var.region
  media_bucket_name                        = "vwu-magento-stg-media"
  assets_bucket_name                       = "vwu-magento-stg-assets"
  opensearch_snapshot_bucket_name          = "vwu-magento-stg-opensearch-snapshots"
  force_destroy                            = false
...
resource "google_secret_manager_secret_iam_member" "opensearch_extra_secret_access" {
  for_each = setsubtract(
    setunion(local.opensearch_secret_ids, toset(["magento-stg-opensearch-app-password"])),
...
module "opensearch" {
  ...
  app_password_secret_id        = "magento-stg-opensearch-app-password"
  ...
  cluster_tag                   = "magento-stg-opensearch"
  labels                        = local.labels
...
module "compute_runtime" {
  ...
  db_password_secret_id         = "magento-stg-db-app-password"
  db_release_password_secret_id = "magento-stg-db-release-password"
